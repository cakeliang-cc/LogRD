...
class LogDataset(Dataset):
    def __init__(self, vocab, log_corpus, time_corpus,  embedding_arr, seq_len, corpus_lines=None, encoding="utf-8", on_memory=True, predict_mode=False, mask_ratio=0.15):
        """
    ...
    def __getitem__(self, idx):
        ...
        # 保存原始序列（在 mask 处理之前），用于后续计算 run 权重
        original_sequence = new_k.copy()
        ...
        # 对原始序列进行相同的截断操作
        original_sequence = original_sequence[:self.seq_len]
        ...
        # 对原始序列也进行填充，使其长度与处理后的序列一致
        original_sequence = pad_lists_to_length([original_sequence], seq_len)[0]
        ...
        # 将原始序列转换为 tensor，用于后续计算 run 权重
        original_sequence = torch.tensor(original_sequence, dtype=torch.long)

        return k, k_label, cls_label, mask, original_sequence

    def random_item(self, k):

        tokens = k
        output_label = []
        end = self.embedding_arr.shape[0]
