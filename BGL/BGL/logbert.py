...
if __name__ == "__main__":
    ...
    _data = str(Path(dirname).parent / "output" / "bgl")
    _raw = str(Path(_data) / "raw" / "BGL.log")
    proc_parser = subparsers.add_parser(
        "process",
        help="raw BGL.log → BGL_df / 序列 / SimCSE / 白化嵌入（见 bgl_process.py）",
    )
    proc_parser.set_defaults(mode="process")
    proc_parser.add_argument("--raw", type=str, default=_raw, help="BGL.log")
    proc_parser.add_argument("--data-dir", type=str, default=_data, help="输出目录（含 BGL_df、train_vocab 等）")
    proc_parser.add_argument(
        "--skip-raw",
        action="store_true",
        help="已有 BGL_df.csv 时跳过 raw 解析",
    )
    proc_parser.add_argument(
        "--augment",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="白化前 LLM 增广协方差（默认关闭；需 OPENAI_API_KEY）",
    )
    ...
    if args.mode == "process":
        from bgl_process import run_bgl_process

        run_bgl_process(
            raw_log=Path(args.raw),
